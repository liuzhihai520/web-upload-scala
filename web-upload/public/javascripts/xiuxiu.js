...
xiuxiu.lightEditor = xiuxiu.lightEditor || {
    url: "http://open.web.meitu.com/sources/light/PhotoEditor.swf?version=201405281145",
...
xiuxiu.richEditor = xiuxiu.richEditor || {
    url: "http://xiuxiu.web.meitu.com/Main.swf?v=2.6.101",
...
xiuxiu.avatarEditor = xiuxiu.avatarEditor || {
    url: "http://open.web.meitu.com/sources/avatar/avatar.swf?version=201405071529",
...
xiuxiu.embedSWF = function(b, c, f, l, e) {
    var a;
    xiuxiu.container = b;
    if ($("#" + xiuxiu.container).length > 0) {
        $("#" + xiuxiu.container).html('<p><a href="http://www.adobe.com/go/getflashplayer"><img alt="Get Adobe Flash player" src="http://wwwimages.adobe.com/www.adobe.com/images/shared/download_buttons/get_flash_player.gif"></a></p>');
    }
        ...
        if (xiuxiu.editorType == xiuxiu.richEditor.type) {
            ...
            if (xiuxiu.editorType == xiuxiu.daitoutie.type) {
                ...
                if (xiuxiu.editorType == xiuxiu.avatarEditor.type) {
                    ...
                    l = l || xiuxiu.avatarEditor.height
                } else {
    ...
    var g = {};
    for (var j in xiuxiu.flashvars.defaultvars) {
        g[j] = xiuxiu.flashvars.defaultvars[j]
    }
    if (xiuxiu.flashvars[xiuxiu.defaultID]) {
        for (j in xiuxiu.flashvars[xiuxiu.defaultID]) {
            g[j] = xiuxiu.flashvars[xiuxiu.defaultID][j]
        }
    }
    if (xiuxiu.flashvars[e]) {
        for (j in xiuxiu.flashvars[e]) {
            g[j] = xiuxiu.flashvars[e][j]
        }
    }
    var k = {};
    for (j in xiuxiu.params) {
        k[j] = xiuxiu.params[j]
    }
    if (xiuxiu.single) {
        var d = xiuxiu.swfs.length;
        for (var h = 0; h < d; h++) {
            swfobject.removeSWF(xiuxiu.swfs[h].id)
        }
        xiuxiu.swfs = []
    }
    swfobject.embedSWF(a, xiuxiu.container, f, l, "10.2.0", "http://open.web.meitu.com/sources/expressInstall.swf", g, k, xiuxiu.attributes);
    xiuxiu.swfs.push({
        id: xiuxiu.attributes.id,
        type: xiuxiu.editorType
    });
    xiuxiu._apiTrack("embedSWF_" + c + "_" + f + "_" + l, e)
};
